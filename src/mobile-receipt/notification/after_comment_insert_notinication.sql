...
BEGIN
	...
	IF (SELECT is_alarm_enabled FROM user WHERE user_id = review_author_id) = 'Y' THEN
	    ...
	    VALUES 
		 (review_author_id, 2, CURRENT_TIMESTAMP);     /* notification_type_id는 임의의 알림 메세지를 참조하는 id */
	END if;
END //
DELIMITER ;
...
INSERT 
  INTO comment
  (user_id, review_id, content, created_at, modified_at )
VALUES
('user02', 8, '좋은 리뷰네용', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);     -- (2)
